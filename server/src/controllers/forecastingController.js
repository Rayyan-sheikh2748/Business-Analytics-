function generateForecastChart(horizon) {
  const points = [];
  const histDays = 20;
...
export async function getForecast(req, res) {
  ...
  const mapeMap = { ARIMA: 8.42, Prophet: 11.23, "Linear Regression": 14.67, "Exponential Smoothing": 13.91 };
...
export async function getBreakdown(_req, res) {
  res.json([
...
export async function getHeatmap(_req, res) {
  const heatmap = [];
  for (let w = 1; w <= 5; w++) {
    heatmap.push({
      week: w,
...
export async function getModelComparison(_req, res) {
  res.json([
