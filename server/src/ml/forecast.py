...
# Avoid outputs to stderr/stdout that are not JSON
import warnings
...
def main():
    ...
    sales_list = input_data.get("sales_data", [])
    inventory_list = input_data.get("inventory_data", [])
    horizon = int(input_data.get("horizon", 30))
    model_name = input_data.get("model", "ARIMA")
    product_filter = input_data.get("product_name", None)
    ...
    for w in range(1, 6):
        # Add slight variation per week to look realistic and responsive
        heatmap_data.append({
            "week": w,
    ...
    # ─── PRODUCT INVESTMENT RECOMMENDATION ENGINE ───
    recommendations = []
    if len(sales_df) > 0:
        # Group by product
        ...
        for name, group in grouped_prod:
            ...
            # Growth rate calculation: compare last 30 days vs previous 30 days
        ...
        if len(product_stats_df) > 0:
            # Sort products by demand & growth
            ...
            for index, row in top_growth.iterrows():
                ...
                # Predict forecasted demand for this product
                ...
                # Check condition for Invest More or Reduce Investment
                if g_rate >= 5.0 or (row['totalSales'] > product_stats_df['totalSales'].median() and p_profit > product_stats_df['profit'].median()):
                    ...
                    reason = f"Predicted demand is expected to increase by {int(max(5, g_rate + 2))}% next month based on growth velocity."
                    if forecasted_demand_next_month > 0:
    ...
    for item in inventory_list:
        ...
        # Get historical daily series for this product
        prod_daily = process_sales_series(sales_df, p_name)
        
        # Train ARIMA / RF model
        if len(prod_daily) > 0:
            prod_fc_vals, _, _, _ = run_arima(prod_daily, 90) # Forecast next 90 days
        ...
        if avg_daily_demand_val > 0:
            ...
            days_to_depletion = 999 # infinite/no depletion
            
        # Recommended reorder quantity
        # Standard safety stock formula: (Max Daily Sales * Max Lead Time) - (Avg Daily Sales * Avg Lead Time)
        # Simplified: safety stock = 5 days of avg demand. Reorder quantity = 30 days of avg demand + safety stock - current_stock
        safety_stock = int(np.ceil(avg_daily_demand_val * 7))
        lead_time_demand = int(np.ceil(avg_daily_demand_val * 10)) # Assume 10 days lead time
        recommended_reorder_qty = max(0, lead_time_demand + safety_stock - current_stock)
        
        # Check for potential shortage
        has_shortage = False
        shortage_day = None
        cumulative_demand = 0
